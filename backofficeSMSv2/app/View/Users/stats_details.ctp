<div class="users form">
	...
	<form>
		<select id="chartType" name="chartType">
			<option value="0">L&iacute;neas</option>
			<option value="1">Barras</option>
		</select>
	</form>
	<div id="chart_div" style="width:790px; height:300px;"></div>
	<table>
		<tr>
...
<script type="text/javascript">
	...
	function setChart() {
		var data = google.visualization.arrayToDataTable([
			['Fecha', 'Digitel', 'Movilnet', 'Movistar', 'Fijos']
			<?php $x = 0; ?>
			<?php foreach ($report as $r): ?>
			<?php echo ',['; ?>
			<?php echo "'".substr($r['date'], 8, 2).'/'.substr($r['date'], 5, 2)."',"; ?>
			<?php echo $r['digitel'].','; ?>
			<?php echo $r['movilnet'].','; ?>
			<?php echo $r['movistar'].','; ?>
			<?php echo $r['others']; ?>
		...
		var options = {
			chartArea: {left:40},
			series: [{color:'red'}, {color:'orange'}, {color:'green'}, {color:'blue'}]
		};
		chartData = data;
